...
export default function ReviewDesk({ sessions, selectedId, onSelect, api, onRefresh }: { sessions: Row[]; selectedId: string; onSelect: (id: string) => void; api: (path: string, options?: RequestInit) => Promise<any>; onRefresh: () => Promise<void> }) {
  ...
  const deskState = humanState(assessment?.payload?.humanReview || [])
  const deskFindings = criteria.filter((c: Row) => c.status === 'INCUMPLE')
  const deskPending = deskFindings.filter((c: Row) => !deskState.decisions[c.name] || deskState.decisions[c.name].decision === 'defer')
  ...
  return <div className="review-desk">
    ...
    <div className={'desk-panels mobile-' + mobile}>
      ...
      {!assessment ? <p className="desk-context">Esta conversación todavía no tiene una evaluación. Puedes leer el chat completo a la derecha.</p> : <>
        ...
        <details className="desk-summary-details"><summary>Resumen de la evaluación</summary><p className="desk-summary">{verdict.summary}</p></details><p className="desk-context">Cobertura {verdict.coverage}{deskFindings.length ? ` · ${deskFindings.length - deskPending.length} de ${deskFindings.length} hallazgos resueltos` : ' · Sin hallazgos de IA'}</p>
