    ...
    <div className={'desk-panels mobile-' + mobile}>
      ...
      {!assessment ? <p className="desk-context">Esta conversación todavía no tiene una evaluación. Puedes leer el chat completo a la derecha.</p> : <>
        ...
        <div className="desk-overview">
          ...
          <details className="desk-summary-details"><summary>Los {criteria.length} criterios de la IA</summary><div className="guide-steps">{criteria.map((c: Row) => <button key={c.name} disabled={busy} aria-pressed={criterion === c.name} onClick={() => { setHumanEvidence([]); setCriterion(c.name); setReference(0) }}>{criterionNames[c.name]}<small>{statuses[c.status]}</small></button>)}</div></details>
        </div>
